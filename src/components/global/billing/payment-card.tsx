  ...
  return (
    <div
      className={cn(
        label !== current
          ? "bg-in-active"
          : "bg-gradient-to0r from-indigo-500 via-purple-500 to-pink-500",
    ...
    >
      ...
      >
        ...
        {label === "PRO" ? (
          ...
          <p className="font-bold mt-2 text-3xl text-text-secondary">
            Standard
          </p>
        )}
        {label === "PRO" ? (
        ...
        {PLANS[label === "PRO" ? 1 : 0].features.map((i) => (
          <p key={i} className="mt-2 text-muted-foreground flex gap-2">
            <CircleCheck className="to-indigo-500" />
            {i}
          </p>
        ))}
        ...
        {landing ? (
          ...
          <Button
            className="rounded-full mt-5 bg-background-80 text-white hover:text-background-80 cursor-pointer"
